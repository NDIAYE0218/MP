  ...
  constructor(private marcheservice: MarcheService, private router: Router, private params: ActivatedRoute) {
    router.events.forEach((event) => {
      if (event instanceof NavigationEnd) {
        ...
        if (this.option == "directions") { this.findData_finances(0); this.intit_pie = 0 }
        if (this.option == "services") { this.findData_finances(1); this.intit_pie = 0 }
        if (this.option == "années") { this.findData_finances(2); this.intit_pie = 0 }
        if (this.option == "type de Marché") { this.findData_finances(3); this.intit_pie = 1 }
        if (this.option == "forme de Marché") { this.findData_finances(4); this.intit_pie = 1 }
        if (this.option == "type de procedure") { this.findData_finances(5); this.intit_pie = 1 }
        if (this.option == "nature du Marché") { this.findData_finances(6); this.intit_pie = 1 }
      }
    })
  }
  ngOnInit() {
  ...
  findData_finances(option) {
    this.marcheservice.Statistiques(option).subscribe((data: any) => {
      ...
      if (option < 3)
        ...
        this.data_histo = [{ data: data.Montant_min, label: "Nombre de procedure" }]
    })
  }
  change_vue(option) {
